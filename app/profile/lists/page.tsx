...
export default function ListsPage() {
  ...
  const load = useCallback(() => {
    if (!user) return;
    const data = getLists(user.uid) as StoreItem[];
    setItems(data);
  }, [user]);
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (!e.key) return;
      if (e.key.includes('tf_lists_')) load();
    };
    window.addEventListener('lists:updated' as any, onUpdated);
    window.addEventListener('storage', onStorage);
    return () => {
      window.removeEventListener('lists:updated' as any, onUpdated);
  ...
  if (!user) {
    return (
      <div className="max-w-3xl mx-auto p-6 text-center">
        <p className="text-white/80 mb-4">Sign in to manage your lists.</p>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <header className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-white flex items-center gap-2">
          <ListIcon className="h-5 w-5 text-sky-400" /> Lists
        </h1>
      ...
      {items.length === 0 ? (
        <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-center">
          <p className="text-white/80 mb-2">You haven’t added anything to your lists.</p>
          <p className="text-sm text-white/60">Use the plus icon on any movie or TV page.</p>
        </div>
      ) : (
